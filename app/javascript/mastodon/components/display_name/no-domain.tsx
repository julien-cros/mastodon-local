...
> = ({ account, className, children, localDomain: _, ...props }) => {
  return (
    ...
    >
      <bdi>
        {account ? (
          /* 1. LE CONTENEUR PRINCIPAL (Flexbox) */
          /* inline-flex : se comporte comme du texte pour ne pas casser le layout autour */
          /* maxWidth: 100% : Interdit de dépasser la largeur du parent */
          <span
            style={{
              display: 'inline-flex',
              alignItems: 'center',
          ...
          >
            {/* 2. LE NOM (Doit pouvoir être coupé) */}
            {/* flex: '0 1 auto' -> Peut rétrécir (1) mais pas grandir (0) */}
            {/* minWidth: 0 -> L'astuce magique pour que le text-overflow fonctionne dans un flex */}
            <span
              style={{
                flex: '0 1 auto',
                overflow: 'hidden',
            ...
            {/* 3. LE BADGE (Ne doit jamais rétrécir) */}
            {account.get('is_verified') && (
              <span
                title='Compte vérifié'
                style={{
                  flex: '0 0 auto', // Ne rétrécit jamais, ne grandit jamais
                  display: 'flex',
                  alignItems: 'center',
                  marginLeft: '4px',
                  color: '#1da1f2',
